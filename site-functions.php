...
if ( ! function_exists( 'site_user_activity_table_create' ) ) {
	/**
	 * Create table "wp_user_activity" when activate plugin.
	 */
	function site_user_activity_table_create() {
		global $wpdb;
		$plugin_data     = get_plugin_data( WP_PLUGIN_DIR . '/site-activity-log/site_activity_log.php', $markup = true, $translate = true );
		$current_version = $plugin_data['Version'];
		$table_name      = $wpdb->prefix . 'sitep_user_activity';
		// table is not created. you may create the table here.
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}sitep_user_activity'" ) != $table_name ) {
			$create_table_query = "CREATE TABLE {$wpdb->prefix}sitep_user_activity (uactid bigint(20) unsigned NOT NULL auto_increment,post_id int(20) unsigned NOT NULL,post_title varchar(250) NOT NULL,user_id bigint(20) unsigned NOT NULL default '0',user_name varchar(50) NOT NULL,user_role varchar(50) NOT NULL,user_email varchar(50) NOT NULL,ip_address varchar(50) NOT NULL,modified_date datetime NOT NULL default '0000-00-00 00:00:00',object_type varchar(50) NOT NULL default 'post',action varchar(50) NOT NULL,PRIMARY KEY (uactid))";
			require_once ABSPATH . 'wp-admin/includes/upgrade.php';
			dbDelta( $create_table_query );
		}
		update_option( 'site_version', $current_version );
	}
}
add_action( 'activate_plugin', 'site_user_activity_table_create' );


if ( ! function_exists( 'site_user_activity_add' ) ) {
	/**
	 ...
	 */
	function site_user_activity_add( $post_id, $post_title, $obj_type, $current_user_id, $current_user, $user_role, $user_mail, $modified_date, $ip, $action ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'sitep_user_activity';
...
if ( ! function_exists( 'site_get_activity_function' ) ) {
	/**
	 * Get activity
	 *
	 * @param string $action current user's activity name.
	 ...
	 * @param string $post_title Post Title.
	 */
	function site_get_activity_function( $action, $obj_type, $post_id, $post_title ) {
		$current_user_id           = '';
		...
		site_user_activity_add( $post_id, $post_title, $obj_type, $current_user_id, $current_user_display_name, $user_role, $user_mail, $modified_date, $ip, $action );
	}
}

if ( ! function_exists( 'site_get_logout_activity_function' ) ) {
	/**
	 * Get logout activity
	 *
	 * @param string $action current user's activity name.
	 ...
	 * @param string $post_title Post Title.
	 */
	function site_get_logout_activity_function( $action, $obj_type, $post_id, $post_title ) {
		$current_user_id           = '';
		$current_user_display_name = '';
		$user_mail                 = '';
		$user_role                 = '';
...
if ( ! function_exists( 'site_allow_ip' ) ) {
	/**
	 * Function to check if Ipaddress is allowed.
	 */
	function site_allow_ip() {
...
if ( ! function_exists( 'site_shook_wp_login' ) ) :
	/**
	 ...
	 */
	function site_shook_wp_login( $user_login, $user ) {
		global $wpdb;
		$action          = 'logged in';
		$obj_type        = 'user';
		$user_mail       = $user->user_email;
...
if ( ! function_exists( 'site_shook_wp_logout' ) ) :
	/**
	 ...
	 * @param string $user Current User.
	 */
	function site_shook_wp_logout( $redirect_to, $requested_redirect_to, $user ) {
		$action   = 'logged out';
		$obj_type = 'user';
		...
		site_get_logout_activity_function( $action, $obj_type, $post_id, $post_title );
		$requested_redirect_to = wp_login_url( get_permalink() );
		return $requested_redirect_to;
	}
endif;

if ( ! function_exists( 'site_shook_delete_user' ) ) :
	/**
	 * Get activity for the delete user
	 *
	 * @param int $user User ID.
	 */
	function site_shook_delete_user( $user ) {
		$action     = 'delete user';
		$obj_type   = 'user';
		$post_id    = $user;
...
if ( ! function_exists( 'site_shook_user_register' ) ) :
	/**
	 * Get activity for the registered user
	 *
	 * @param int $user User ID.
	 */
	function site_shook_user_register( $user ) {
...
if ( ! function_exists( 'site_shook_profile_update' ) ) :
	/**
	 * Get activity for the user - update profile
	 *
	 * @param int $user User ID.
	 */
	function site_shook_profile_update( $user ) {
...
if ( ! function_exists( 'site_shook_add_attachment' ) ) :
	/**
	 * Get activity for the user - add attach media file
	 *
	 * @param int $attach Post ID.
	 */
	function site_shook_add_attachment( $attach ) {
...
if ( ! function_exists( 'site_shook_edit_attachment' ) ) :
	/**
	 * Get activity for the user - edit attach media file
	 *
	 * @param int $attach Post ID.
	 */
	function site_shook_edit_attachment( $attach ) {
		$post_id    = $attach;
		$post_title = get_the_title( $post_id );
...
if ( ! function_exists( 'site_shook_delete_attachment' ) ) :
	/**
	 * Get activity for the user - delete attach media file
	 *
	 * @param int $attach Post ID.
	 */
	function site_shook_delete_attachment( $attach ) {
		$post_id    = $attach;
		$post_title = get_the_title( $post_id );
...
if ( ! function_exists( 'site_shook_delete_post' ) ) :
	/**
	 * Get activity for the post - delete post file
	 *
	 * @param int $post Post ID.
	 */
	function site_shook_delete_post( $post ) {
		global $post_type;
...
if ( ! function_exists( 'site_shook_wp_insert_comment' ) ) :
	/**
	 * Get activity for the user - Insert Comment
	 *
	 * @param int $comment Comment ID.
	 */
	function site_shook_wp_insert_comment( $comment ) {
...
if ( ! function_exists( 'site_shook_edit_comment' ) ) :
	/**
	 * Get activity for the user - Edit Comment
	 *
	 * @param int $comment Comment ID.
	 */
	function site_shook_edit_comment( $comment ) {
...
if ( ! function_exists( 'site_shook_trash_comment' ) ) :
	/**
	 * Get activity for the user - Trash Comment
	 *
	 * @param int $comment Comment ID.
	 */
	function site_shook_trash_comment( $comment ) {
...
if ( ! function_exists( 'site_shook_spam_comment' ) ) :
	/**
	 * Get activity for the user - Spam Comment
	 *
	 * @param int $comment Comment ID.
	 */
	function site_shook_spam_comment( $comment ) {
		$action     = 'spam comment';
		$obj_type   = 'comment';
		$comment_id = $comment;
		$com        = get_comment( $comment_id );
...
if ( ! function_exists( 'site_shook_unspam_comment' ) ) :
	/**
	 * Get activity for the user - Unspam Comment
	 *
	 * @param int $comment Comment ID.
	 */
	function site_shook_unspam_comment( $comment ) {
...
if ( ! function_exists( 'site_shook_delete_comment' ) ) :
	/**
	 * Get activity for the user - Delete Comment
	 *
	 * @param int $comment Comment ID.
	 */
	function site_shook_delete_comment( $comment ) {
...
if ( ! function_exists( 'site_hook_status_comment' ) ) {
	/**
	 ...
	 */
	function site_hook_status_comment( $new_status, $old_status, $comment_id ) {
		$obj_type                          = 'Comment';
		$com                               = get_comment( $comment_id );
		$post_id                           = $com->comment_post_ID;
		$comment_detail_ary                = array();
		$comment_detail_ary['site_comment'] = $com->comment_content;
		$post_title                        = get_the_title( $post_id );
		$post_link                         = get_the_permalink( $post_id );
...
add_action( 'transition_comment_status', 'site_hook_status_comment', 10, 3 );

if ( ! function_exists( 'site_shook_created_term' ) ) :
	/**
	 ...
	 */
	function site_shook_created_term( $term, $taxonomy ) {
...
if ( ! function_exists( 'site_shook_edited_term' ) ) :
	/**
	 ...
	 * @param string $taxonomy taxonomy name.
	 */
	function site_shook_edited_term( $term, $ttid, $taxonomy ) {
...
if ( ! function_exists( 'site_shook_delete_term' ) ) :
	/**
	 ...
	 * @param string $taxonomy taxonomy name.
	 */
	function site_shook_delete_term( $term, $taxonomy ) {
		if ( 'nav_menu' === $taxonomy ) {
			return;
		}
		...
		if ( $taxonomy && ! is_wp_error( $taxonomy ) ) {
			...
			$termname         = get_term_by( 'id', $term, $taxonomy );
			$post_title       = $taxonomy_details->label . ' - ' . $termname->name;
			site_get_activity_function( $action, $obj_type, $term, $post_title );
		}
	}

endif;

if ( ! function_exists( 'site_shook_wp_update_nav_menu' ) ) :
	/**
	 * Get activity for the user - Update navigation menu
	 *
	 * @param int $menu Post ID.
	 */
	function site_shook_wp_update_nav_menu( $menu ) {
...
if ( ! function_exists( 'site_shook_wp_create_nav_menu' ) ) :
	/**
	 * Get activity for the user - Create navigation menu
	 *
	 * @param int $menu Post ID.
	 */
	function site_shook_wp_create_nav_menu( $menu ) {
...
if ( ! function_exists( 'site_shook_switch_theme' ) ) :
	/**
	 * Get activity for the user - Switch Theme
	 *
	 * @param string $theme Post Title.
	 */
	function site_shook_switch_theme( $theme ) {
		$action     = 'switch theme';
		$obj_type   = 'theme';
		$post_id    = '';
		$post_title = $theme;
...
if ( ! function_exists( 'site_shook_customize_save' ) ) :
	/**
	 * Get activity for the user - Customize Theme.
	 */
	function site_shook_customize_save() {
...
if ( ! function_exists( 'site_shook_activated_plugin' ) ) :
	/**
	 * Get activity for the user - Activate Plugin
	 *
	 * @param string $plugin Post Title.
	 */
	function site_shook_activated_plugin( $plugin ) {
...
if ( ! function_exists( 'site_hook_before_edit_post' ) ) {
	/**
	 * Hook Before Edit Post
	 *
	 * @param string $post_ID Post ID.
	 */
	function site_hook_before_edit_post( $post_ID ) {
...
add_action( 'pre_post_update', 'site_hook_before_edit_post', 10 );

if ( ! function_exists( 'site_shook_transition_post_status' ) ) {
	/**
	 * Get activity for the user - Activate Plugin
	 *
	 * @param string $post_id post ID.
	 ...
	 */
	function site_shook_transition_post_status( $post_id, $post ) {
		global $old_post_data;
...
if ( ! function_exists( 'site_shook_deactivated_plugin' ) ) :
	/**
	 * Get activity for the user - Deactivate Plugin
	 *
	 * @param string $plugin Post Title.
	 */
	function site_shook_deactivated_plugin( $plugin ) {
		$plugin_data = get_plugin_data( WP_PLUGIN_DIR . '/' . $plugin, true, false );
		$post_title  = $plugin_data['Name'];
...
if ( ! function_exists( 'site_shook_delete_plugin' ) ) {
	/**
	 * Get activity for the user - Delete Plugin.
	 *
	 * @param string $plugin file name.
	 */
	function site_shook_delete_plugin( $plugin ) {
...
if ( ! function_exists( 'site_shook_export_wp' ) ) :
	/**
	 * Get activity for the user - Export WordPress data
	 *
	 * @param array $args Arguments.
	 */
	function site_shook_export_wp( $args ) {
...
if ( ! function_exists( 'site_shook_theme_deleted' ) ) :
	/**
	 * Get activity for the user - Delete theme.
	 */
	function site_shook_theme_deleted() {
...
add_action( 'wp_login', 'site_shook_wp_login', 20, 2 );
add_filter( 'logout_redirect', 'site_shook_wp_logout', 10, 3 );
add_action( 'delete_user', 'site_shook_delete_user' );
add_action( 'before_delete_post', 'site_shook_delete_post', 10, 1 );
add_action( 'user_register', 'site_shook_user_register' );
add_action( 'profile_update', 'site_shook_profile_update' );
add_action( 'add_attachment', 'site_shook_add_attachment' );
add_action( 'edit_attachment', 'site_shook_edit_attachment' );
add_action( 'delete_attachment', 'site_shook_delete_attachment' );
add_action( 'wp_insert_comment', 'site_shook_wp_insert_comment' );
add_action( 'edit_comment', 'site_shook_edit_comment' );
add_action( 'trash_comment', 'site_shook_trash_comment' );
add_action( 'spam_comment', 'site_shook_spam_comment' );
add_action( 'unspam_comment', 'site_shook_unspam_comment' );
add_action( 'delete_comment', 'site_shook_delete_comment' );
add_action( 'load-nav-menus.php', 'site_shook_wp_update_nav_menu' );
add_action( 'wp_create_nav_menu', 'site_shook_wp_create_nav_menu' );
add_action( 'activated_plugin', 'site_shook_activated_plugin' );
add_action( 'deactivated_plugin', 'site_shook_deactivated_plugin' );
add_action( 'delete_plugin', 'site_shook_delete_plugin' );
add_filter( 'pre_insert_term', 'site_shook_created_term', 10, 2 );
add_action( 'edited_term', 'site_shook_edited_term', 10, 3 );
add_action( 'pre_delete_term', 'site_shook_delete_term', 10, 2 );
add_action( 'switch_theme', 'site_shook_switch_theme' );
add_action( 'customize_save', 'site_shook_customize_save' );
add_action( 'export_wp', 'site_shook_export_wp' );
add_action( 'post_updated', 'site_shook_transition_post_status', 100, 2 );
add_action( 'delete_site_transient_update_themes', 'site_shook_theme_deleted' );

if ( ! function_exists( 'site_shook_wp_login_failed' ) ) :
	/**
	 * Get activity for the user - Login fail
	 *
	 * @param string $user username.
	 */
	function site_shook_wp_login_failed( $user ) {
		...
		if ( 'yes' == $logs_failed_login ) {
			...
			if ( ! empty( $current_user_display_name ) ) {
				$countwploginfailed = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(`uactid`) FROM {$wpdb->prefix}sitep_user_activity where action='login failed' AND post_title=%s", $user ) );
...
add_action( 'wp_login_failed', 'site_shook_wp_login_failed' );

if ( ! function_exists( 'site_shook_widget_update_callback' ) ) :
	/**
	 ...
	 */
	function site_shook_widget_update_callback( $instance, $new_instance, $old_instance, $widget_instance ) {
...
add_filter( 'widget_update_callback', 'site_shook_widget_update_callback', 8, 4 );


if ( ! function_exists( 'site_shook_widget_added_deleted' ) ) {
	/**
	 * Get activity for the user - Widget Add Delete.
	 */
	function site_shook_widget_added_deleted() {
...
add_action( 'sidebar_admin_setup', 'site_shook_widget_added_deleted' );


add_filter( 'wp_privacy_personal_data_exporters', 'site_shook_register_exporter', 10 );

if ( ! function_exists( 'site_shook_register_exporter' ) ) {
	/**
	 * Register Exporter Function.
	 *
	 * @param array $exporters_array Exporters Array.
	 */
	function site_shook_register_exporter( $exporters_array ) {
		$exporters_array['site_exporter'] = array(
			'exporter_friendly_name' => 'UAL exporter',
			'callback'               => 'site_shook_exporter_function',
		);
 ...
 */
function site_shook_exporter_function( $email_address, $iteration = 1 ) {
...
add_filter( 'wp_privacy_personal_data_erasers', 'site_shook_register_my_eraser', 10 );

if ( ! function_exists( 'site_shook_register_my_eraser' ) ) {
	/**
	 * Register Eraser Function.
	 *
	 * @param array $erasers Erasers.
	 */
	function site_shook_register_my_eraser( $erasers ) {
		$erasers['site_eraser'] = array(
			'eraser_friendly_name' => 'UAL eraser',
			'callback'             => 'site_shook_eraser_function',
		);
		return $erasers;
	}
}
 ...
 * @param string $iteration Iteration.
 */
function site_shook_eraser_function( $email_address, $iteration = 1 ) {
...
if ( ! function_exists( 'site_test_input' ) ) {
	/**
	 * Input validation function
	 *
	 * @param string $data input data.
	 */
	function site_test_input( $data ) {
		$data = trim( $data );
...
if ( ! function_exists( 'site_admin_notice_message' ) ) {
	 ...
	 */
	function site_admin_notice_message( $class, $message ) {
		?>
...
if ( ! function_exists( 'site_settings_link' ) ) {
	/**
	 * Display success or error message
	 *
	 ...
	 */
	function site_settings_link( $actions, $plugin_file ) {
		static $plugin;
		if ( empty( $plugin ) ) {
			$plugin = dirname( plugin_basename( __FILE__ ) ) . '/site_activity_log.php';
		}
		if ( $plugin_file == $plugin ) {
			$settings_link = '<a href="' . admin_url( 'admin.php?page=general_settings_menu' ) . '">' . esc_html__( 'Settings', 'site-activity-log' ) . '</a>';
			array_unshift( $actions, $settings_link );
		}
		return $actions;
	}
}
add_filter( 'plugin_action_links', 'site_settings_link', 10, 2 );

if ( ! function_exists( 'site_plugin_upgrade_notice_screen' ) ) {
	/**
	 * Upgrade Notice Screen.
	 */
	function site_plugin_upgrade_notice_screen() {
		$screen = get_current_screen();
		if ( isset( $_GET['page'] ) && ( 'user_action_log' == $_GET['page'] || 'general_settings_menu' == $_GET['page'] ) ) {
			add_action( 'admin_notices', 'site_plugin_upgrade_notice' );
		}
	}
}
add_action( 'current_screen', 'site_plugin_upgrade_notice_screen' );

if ( ! function_exists( 'site_plugin_upgrade_notice' ) ) {
	/**
	 * Add notice at admin side.
	 */
	function site_plugin_upgrade_notice() {
		global $current_user;
		$user_id = $current_user->ID;
		/* Check that the user hasn't already clicked to ignore the message. */
		if ( ! get_user_meta( $user_id, 'site_plugin_upgrade_notice' ) && current_user_can( 'manage_options' ) ) {
			?>
			...
			?>
				...
				<p>
					<?php esc_html_e( 'Want more Site Activity Log features?', 'site-activity-log' ); ?>
					<a href="https://codecanyon.net/item/site-activity-log-pro-for-wordpress/18201203?ref=sitetech" target="_blank" style="text-decoration: underline">
						<strong><?php esc_html_e( 'Upgrade to PRO', 'site-activity-log' ); ?></strong>
					</a>
				</p>
				<p>
					<a href="http://useractivitylog.sitetech.com/" target="_blank"><strong><?php esc_html_e( 'Live Preview', 'site-activity-log' ); ?></strong></a> |
					<a href="<?php echo esc_url( $genre_url ); ?>"><strong><?php esc_html_e( 'Dismiss This Notice', 'site-activity-log' ); ?></strong></a>
				</p>
			</div>
			<?php
		}
...
if ( ! function_exists( 'site_ignore_upgrade_notice' ) ) {
	/**
	 * Add user meta for ignore notice.
	 */
	function site_ignore_upgrade_notice() {
		global $current_user;
		$user_id = $current_user->ID;
		/* If user clicks to ignore the notice, add that to their user meta. */
		if ( isset( $_GET['site_plugin_upgrade_notice'] ) && '0' == $_GET['site_plugin_upgrade_notice'] ) {
			add_user_meta( $user_id, 'site_plugin_upgrade_notice', 'true', true );
		}
	}
}
add_action( 'admin_init', 'site_ignore_upgrade_notice' );

add_action( 'init', 'site_filter_user_role' );

if ( ! function_exists( 'site_filter_user_role' ) ) :
	/**
	 * Filter user Roles.
	 */
	function site_filter_user_role() {
		$paged     = 1;
...
if ( ! function_exists( 'site_admin_scripts' ) ) {
	/**
	 * Admin scripts.
	 */
	function site_admin_scripts() {
		?>
		<script>
		...
		$plugin_data     = get_plugin_data( WP_PLUGIN_DIR . '/site-activity-log/site_activity_log.php', $markup = true, $translate = true );
		$current_version = $plugin_data['Version'];
		$old_version     = get_option( 'site_version' );
		if ( $old_version != $current_version ) {
			update_option( 'site_version', $current_version );
		}

		wp_register_style( 'site-style-css', plugins_url( 'css/style.css', __FILE__ ), array(), '1.0' );
		wp_enqueue_style( 'site-style-css' );
		...
		wp_localize_script(
			...
			'ualpJSObject',
			array(
				'ip_address' => esc_html__( 'IP address', 'site_activity_log_pro' ),
				'network'    => esc_html__( 'Network', 'site_activity_log_pro' ),
				'city'       => esc_html__( 'City', 'site_activity_log_pro' ),
				'region'     => esc_html__( 'Region', 'site_activity_log_pro' ),
				'country'    => esc_html__( 'Country', 'site_activity_log_pro' ),
				'log_delete' => esc_html__( 'Select Logs to delete', 'site_activity_log_pro' ),
...
if ( $wpp_version > 3.8 ) {
	if ( ! function_exists( 'wp_custom_star_rating_site_activity_log' ) ) {
		/**
		 * Get rating star and total downloads of current plugin.
		 *
		 * @param array $args Arugments.
		 */
		function wp_custom_star_rating_site_activity_log( $args = array() ) {
			$plugins  = '';
			$response = '';
			$args     = array(
				'author' => 'sitetech',
			...
			$current_slug = 'site-activity-log';
			if ( $plugins ) {
...
if ( ! function_exists( 'get_total_downloads_site_activity_log_plugin' ) ) {
	/**
	 * Get total downloads of current plugin.
	 */
	function get_total_downloads_site_activity_log_plugin() {
		$plugins  = '';
		$response = '';
		$args     = array(
			'author' => 'sitetech',
...
add_action( 'user_register', 'site_enable_user_notification_at_login' );
if ( ! function_exists( 'site_enable_user_notification_at_login' ) ) {
	/**
	 * Enable user notification of email at login
	 *
	 * @param int $user_id user ID.
	 */
	function site_enable_user_notification_at_login( $user_id ) {
...
add_action( 'plugins_loaded', 'site_latest_activity_logs' );
if ( ! function_exists( 'site_latest_activity_logs' ) ) {
	/**
	 * Add Admin Dashboard Widget - News from Site Tech.
	 */
	function site_latest_activity_logs() {
		// Register the new dashboard widget with the 'wp_dashboard_setup' action.
		add_action( 'wp_dashboard_setup', 'site_latest_logs' );
		if ( ! function_exists( 'site_latest_logs' ) ) {
			/**
			 * Latest Logs.
			 */
			function site_latest_logs() {
				$sitep_allow_stats_report_dashbord_widget = get_option( 'sitep_allow_stats_report_dashbord_widget', '1' );
				$user                                    = wp_get_current_user();
				$roles                                   = $user->roles;
				foreach ( $roles as $role ) {
					if ( 'administrator' == $role ) {
						...
						add_meta_box( 'site_dashboard_widget', __( 'Latest Site Activity Logs', 'site-activity-log' ), 'site_dashboard_widget', 'dashboard', 'normal', 'high' );
						if ( 1 == $sitep_allow_stats_report_dashbord_widget ) {
							add_meta_box( 'site_dashboard_widget', esc_html__( ' Site Activity Logs Stats', 'site-activity-log' ), 'site_dashboard_log_stats', 'dashboard', 'normal', 'high' );
						}
					}
				}
			}
		 ...
		 * Dashboard Log Stats.
		 */
		function site_dashboard_log_stats() {
			global $wpdb;
			$table_name               = $wpdb->prefix . 'sitep_user_activity';
			$stats_labels             = array();
			...
			foreach ( $period as $dt ) {
				$datef        = _x( 'M j', 'stats: date in rows per day chart', 'site-activity-log' );
				...
				$result_log_query = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}sitep_user_activity WHERE modified_date >= %s  AND  modified_date < %s ORDER BY modified_date desc", $start_date, $enddate ) );
			...
			?>
			<input type="hidden" class="sitep_chat_label" value="<?php echo esc_attr( $st_labels ); ?>" />
			<input type="hidden" class="sitep_chat_date_value" value="<?php echo esc_attr( $arr_dataset ); ?>" />
			...
			<div style="position: relative; height: 0; overflow: hidden; padding-bottom: 50%;">
				...
				var chartLabels =  JSON.parse( jQuery(".sitep_chat_label").val() );
				var sitep_chat_date_value =  JSON.parse( jQuery(".sitep_chat_date_value").val() );
		...
		if ( ! function_exists( 'site_dashboard_widget' ) ) {
			/**
			 * Dashboard Widget.
			 */
			function site_dashboard_widget() {
				?>
				<ul class="site_dashboard_log_table">
					<li>
						<span class="site-dashboard-column-date"><b><?php esc_html_e( 'Date', 'site-activity-log' ); ?></b></span>
						<span class="site-dashboard-column-author"><b><?php esc_html_e( 'Author', 'site-activity-log' ); ?></b></span>
						<span class="site-dashboard-column-activity"><b><?php esc_html_e( 'Activity', 'site-activity-log' ); ?></b></span>
					</li>
					<?php
					global $wpdb;
					$get_data = $wpdb->get_results( $wpdb->prepare( "SELECT * from {$wpdb->prefix}sitep_user_activity ORDER BY modified_date desc LIMIT %d", 5 ) );
					if ( $get_data ) {
						$srno = 1;
						foreach ( $get_data as $data ) {
							?>
							<li>
								<span class="site-dashboard-column-date">
								<?php
								...
								<span class="site-dashboard-column-author column-author">
								<?php
									global $wp_roles;
								...
								<span class="site-dashboard-column-activity">
									<?php
						...
						echo '<li>';
						echo '<div>' . esc_html__( 'No Log found.', 'site-activity-log' ) . '</div>';
						echo '</li>';
					}
					?>
...
add_action( 'wp_ajax_site_enable_user_for_notification', 'site_enable_user_for_notification' );

if ( ! function_exists( 'site_enable_user_for_notification' ) ) {
	/**
	 * Enable User For Notification.
	 */
	function site_enable_user_for_notification() {
...
add_action( 'wp_ajax_site_submit_optin', 'site_submit_optin' );
if ( ! function_exists( 'site_submit_optin' ) ) {
	/**
	 * Submit Options.
	 */
	function site_submit_optin() {
		global $wpdb, $wp_version;
		$site_submit_type = '';
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ajax-nonce' ) ) {
			if ( isset( $_POST['email'] ) ) {
				$site_email = sanitize_text_field( wp_unslash( $_POST['email'] ) );
			} else {
				$site_email = get_option( 'admin_url' );
			}
			if ( isset( $_POST['type'] ) ) {
				$site_submit_type = sanitize_text_field( wp_unslash( $_POST['type'] ) );
			}
			if ( 'submit' == $site_submit_type ) {
				$status_type   = get_option( 'site_is_optin' );
				$theme_details = array();
				if ( $wp_version >= 3.4 ) {
				...
				$plugin_data                           = array();
				$plugin_data['plugin_name']            = 'Site Activity Log';
				$plugin_data['plugin_slug']            = 'site-activity-log';
				...
				$plugin_data['site_email']             = false !== $site_email ? esc_attr( $site_email ) : get_option( 'admin_email' );
				...
				$url                                   = 'https://www.sitetech.com/analytics/';
				...
				update_option( 'site_is_optin', 'yes' );
			} elseif ( 'cancel' == $site_submit_type ) {
				update_option( 'site_is_optin', 'no' );
			} elseif ( 'deactivate' == $site_submit_type ) {
				$status_type   = get_option( 'site_is_optin' );
				$theme_details = array();
				if ( $wp_version >= 3.4 ) {
					$active_theme                   = wp_get_theme();
				...
				$l_name                                = $current_user->user_lastname;
				$plugin_data['site_user_name']         = esc_attr( $f_name ) . ' ' . esc_attr( $l_name );
				$plugin_data['site_email']             = false !== $site_email ? $site_email : get_option( 'admin_email' );
				$plugin_data['site_wordpress_version'] = $wp_version;
				$plugin_data['site_php_version']       = esc_attr( phpversion() );
				$plugin_data['site_mysql_version']     = $wpdb->db_version();
				$plugin_data['site_max_input_vars']    = ini_get( 'max_input_vars' );
...
if ( ! function_exists( 'site_subscribe_mail' ) ) {
	/**
	 * Subscribe email form.
	 */
	function site_subscribe_mail() {
		?>
		<div id="sol_deactivation_widget_cover_ual" style="display:none;">
			<div class="sol_deactivation_widget">
				<h3><?php esc_html_e( 'If you have a moment, please let us know why you are deactivating. We would like to help you in fixing the issue.', 'site-activity-log' ); ?></h3>
				<form id="frmDeactivationual" name="frmDeactivation" method="post" action="">
					<ul class="sol_deactivation_reasons_ul">
						<?php $i = 1; ?>
						<li>
							<input class="sol_deactivation_reasons" checked name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'I am going to upgrade to PRO version', 'site-activity-log' ); ?></label>
						</li>
						<?php $i++; ?>
						<li>
							<input class="sol_deactivation_reasons" name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'The plugin suddenly stopped working', 'site-activity-log' ); ?></label>
						</li>
						<?php $i++; ?>
						<li>
							<input class="sol_deactivation_reasons" name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'The plugin was not working', 'site-activity-log' ); ?></label>
						</li>
						<?php $i++; ?>
						<li>
							<input class="sol_deactivation_reasons" name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'Found other better plugin than this plugin', 'site-activity-log' ); ?></label>
						</li>
						<?php $i++; ?>
						<li>
							<input class="sol_deactivation_reasons" name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'The plugin broke my site completely', 'site-activity-log' ); ?></label>
						</li>
						<?php $i++; ?>
						<li>
							<input class="sol_deactivation_reasons" name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'No any reason', 'site-activity-log' ); ?></label>
						</li>
						<?php $i++; ?>
						<li>
							<input class="sol_deactivation_reasons" name="sol_deactivation_reasons_ual" type="radio" value="<?php echo esc_attr( $i ); ?>" id="site_reason_<?php echo esc_attr( $i ); ?>">
							<label for="site_reason_<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'Other', 'site-activity-log' ); ?></label><br/>
					...
					<p>
						<input type='checkbox' class='site_agree' id='site_agree_gdpr_deactivate' value='1' />
						<label for='site_agree_gdpr_deactivate' class='site_agree_gdpr_lbl'><?php esc_html_e( 'By clicking this button, you agree with the storage and handling of your data as mentioned above by this website. (GDPR Compliance)', 'site-activity-log' ); ?></label>
					</p>
					<a onclick='site_submit_optin("deactivate")' class="button button-secondary">
					<?php
					esc_html_e( 'Submit', 'site-activity-log' );
					echo ' &amp; ';
					esc_html_e( 'Deactivate', 'site-activity-log' );
					?>
					</a>
					<input type="submit" name="sbtDeactivationFormClose" id="sbtDeactivationFormCloseual" class="button button-primary" value="<?php esc_attr_e( 'Cancel', 'site-activity-log' ); ?>" />
					<a href="javascript:void(0)" class="site-deactivation" aria-label="<?php esc_attr_e( 'Deactivate Site Activity Log', 'site-activity-log' ); ?>">
																								<?php
																								esc_attr_e( 'Skip', 'site-activity-log' );
																								echo ' &amp; ';
																								esc_attr_e( 'Deactivate', 'site-activity-log' );
																								?>
					</a>
				</form>
				<div class="support-ticket-section">
					<h3><?php esc_html_e( 'Would you like to give us a chance to help you?', 'site-activity-log' ); ?></h3>
					<img src="<?php echo esc_url( SITE_PLUGIN_URL ) . '/images/support-ticket.png'; ?>">
					<a target='_blank' href="<?php echo esc_url( 'http://support.sitetech.com/' ); ?>"><?php esc_html_e( 'Create a support ticket', 'site-activity-log' ); ?></a>
				</div>
			</div>
		</div>
...
add_action( 'admin_head', 'site_subscribe_mail', 11 );

if ( ! function_exists( 'site_update_options' ) ) {
	/**
	 * Function to add log when options updated.
	 */
	function site_update_options() {
		$screen = get_current_screen();
...
add_action( 'admin_head', 'site_update_options', 25 );
 ...
 */
function site_get_ip( $get_ip_type = null ) {
	if ( null === $get_ip_type ) {
