...
if ( ! function_exists( 'site_switch_to_user' ) ) {
	/**
	 ...
	 */
	function site_switch_to_user( $user_id, $old_user_id ) {
...
add_action( 'switch_to_user', 'site_switch_to_user', 15, 2 );

if ( ! function_exists( 'site_switch_back_user' ) ) {
	/**
	 ...
	 * @param int $old_user_id Old User ID.
	 */
	function site_switch_back_user( $user_id, $old_user_id ) {
		$user_info     = get_userdata( $user_id );
		$user_name     = isset( $user_info->display_name ) ? $user_info->display_name : '';
		$old_user_info = get_userdata( $old_user_id );
		$old_user_name = isset( $old_user_info->display_name ) ? $old_user_info->display_name : '';
...
add_action( 'switch_back_user', 'site_switch_back_user', 15, 2 );

if ( ! function_exists( 'site_switch_off_user' ) ) {
	/**
	 * Fires when a user switches off.
	 *
	 * @param int $old_user_id Old User ID.
	 */
	function site_switch_off_user( $old_user_id ) {
		$old_user_info = get_userdata( $old_user_id );
		$old_user_name = isset( $old_user_info->display_name ) ? $old_user_info->display_name : '';
		$obj_type      = 'User Switching';
...
add_action( 'switch_off_user', 'site_switch_off_user', 15, 1 );
