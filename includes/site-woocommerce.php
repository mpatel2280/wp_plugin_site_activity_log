...
if ( ! function_exists( 'site_wc_create_product_variation' ) ) {
	/**
	 * Fires when create product variation.
	 *
	 * @param int $variation_id Variation ID.
	 */
	function site_wc_create_product_variation( $variation_id ) {
...
add_action( 'woocommerce_create_product_variation', 'site_wc_create_product_variation', 15, 1 );

if ( ! function_exists( 'site_delete_product_variation' ) ) {
	/**
	 * Fires when delete product variation.
	 */
	function site_delete_product_variation() {
		if ( isset( $_POST['delete-variations'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['delete-variations'] ) ), 'security' ) ) {
			...
			foreach ( $variation_ids as $variation_id ) {
				...
				$variation_detail_ary['site_product_name']   = get_the_title( $variation->id );
				$variation_detail_ary['site_variation_data'] = maybe_serialize( $variation_data );
...
add_action( 'wp_ajax_woocommerce_remove_variations', 'site_delete_product_variation' );

if ( ! function_exists( 'site_wc_save_product_variation' ) ) {
	/**
	 ...
	 * @param int $i Value.
	 */
	function site_wc_save_product_variation( $variation_id, $i ) {
		$variation             = '';
		$variation_data        = '';
		$rprice                = '';
		$sprice                = '';
		...
		$variation_detail_ary                                = array();
		$variation_detail_ary['site_variation_sku']           = $sku;
		$variation_detail_ary['site_product_name']            = get_the_title( $variation->id );
		$variation_detail_ary['site_variation_regular_price'] = $rprice;
		$variation_detail_ary['site_variation_sale_price']    = $sprice;
		$variation_detail_ary['site_virtual']                 = $virtual;
		$variation_detail_ary['site_downloadable']            = $downloadable;
		$variation_detail_ary['site_weight']                  = $weight;
		$variation_detail_ary['site_length']                  = $length;
		$variation_detail_ary['site_manage_stock']            = $manage_stock;
		$variation_detail_ary['site_backorders']              = $backorders;
		$variation_detail_ary['site_tax_class']               = $tax_class;
		$variation_detail_ary['site_download_limit']          = $download_limit;
		$variation_detail_ary['site_download_expiry']         = $download_expiry;
		$variation_detail_ary['site_variation_description']   = $variation_description;
		$variation_detail_ary['site_variation_data']          = maybe_serialize( $variation_data );
...
add_action( 'woocommerce_save_product_variation', 'site_wc_save_product_variation', 15, 2 );

if ( ! function_exists( 'site_woocomerce_updated_option' ) ) {
	/**
	 *
	 ...
	 */
	function site_woocomerce_updated_option( $option, $oldvalue, $_newvalue ) {
		global $current_tab;
		if ( isset( $current_tab ) && '' != $current_tab ) {
			$whitelist_options = site_get_wc_options();
...
add_action( 'updated_option', 'site_woocomerce_updated_option', 15, 3 );

if ( ! function_exists( 'site_woocomerce_update_options' ) ) {
	/**
	 * Fires after all woocommerce options updated.
	 */
	function site_woocomerce_update_options() {
		global $current_tab;
		if ( isset( $current_tab ) && '' != $current_tab ) {
			$transient_name   = 'sp_' . $current_tab;
			$settings_page_wc = get_transient( $transient_name );
...
add_action( 'woocommerce_update_options', 'site_woocomerce_update_options', 10 );

if ( ! function_exists( 'site_get_wc_options' ) ) {
	/**
	 * Get all woocommerce options.
	 */
	function site_get_wc_options() {
 ...
 */
class site_Integration_WooCommerce {
	/**
	 * The WC Options.
	 *
	 * @since    1.0.0
	...
	public function init() {
		...
		add_filter( 'ualWhitelistOptions', array( &$this, 'site_wc_whitelist_options' ) );
		add_filter( 'woocommerce_get_settings_pages', array( &$this, 'site_wc_get_settings_pages' ), 9999 );
	}
	 ...
	 */
	public function site_wc_get_settings_pages( $settings ) {
	 ...
	 *
	 * @param string $site_wc_whitelist_options Whitelist options.
	 */
	public function site_wc_whitelist_options( $site_wc_whitelist_options ) {

		if ( ! empty( $this->_wc_options ) ) {
			$site_wc_whitelist_options = array_unique( array_merge( $site_wc_whitelist_options, $this->_wc_options ) );
		}
		return $site_wc_whitelist_options;
	}
	/**
	 * Initialize the class and set its properties.
	 */
...
new site_Integration_WooCommerce();
