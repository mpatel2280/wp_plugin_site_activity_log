...
if ( ! function_exists( 'site_load_settings_page' ) ) {
	/**
	 * Fired when plugin options screen is loaded.
	 *
	 * @param string $a Page.
	 */
	function site_load_settings_page( $a ) {
...
add_action( 'load-settings_page_limit-login-attempts', 'site_load_settings_page', 10, 1 );

if ( ! function_exists( 'site_option_limit_login_lockouts_total' ) ) {
	/**
	 * Fired when plugin options lockouts.
	 *
	 * @param string $value Value.
	 */
	function site_option_limit_login_lockouts_total( $value ) {
		...
		$uactid = site_get_activity_function( $action, $obj_type, $post_id, $post_title );
	}
}

add_filter( 'pre_option_limit_login_lockouts_total', 'site_option_limit_login_lockouts_total', 10, 1 );
