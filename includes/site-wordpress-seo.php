...
if ( ! function_exists( 'site_wpseo_handle_import' ) ) {
	/**
	 * Fires when import data from other SEO plugins.
	 *
	 * @param string $import Import.
	 */
	function site_wpseo_handle_import( $import ) {
...
add_action( 'wpseo_handle_import', 'site_wpseo_handle_import', 10, 1 );

if ( ! function_exists( 'site_wpseo_wpseo_import' ) ) {
	/**
	 * Fires when Export settings.
	 */
	function site_wpseo_wpseo_import() {
...
add_action( 'wpseo_import', 'site_wpseo_wpseo_import' );


if ( ! function_exists( 'site_wpseo_save_auth_code' ) ) {
	/**
	 * Fires when save authentication code.
	 */
	function site_wpseo_save_auth_code() {
...
add_action( 'wp_ajax_wpseo_save_auth_code', 'site_wpseo_save_auth_code' );
...
add_action( 'updated_option', 'site_wp_seo_updated_option', 10, 3 );

if ( ! function_exists( 'site_wp_seo_updated_option' ) ) {
	/**
	 ...
	 */
	function site_wp_seo_updated_option( $option, $old_value = null, $value = null ) {
		...
		foreach ( $seo_removed_array as $seo_removed_key ) {
			...
			$old_value       = apply_filters( 'site_seo_update_array', $old_value, $option );
			$changed_options = array();
			$obj_type        = 'settings';
		if ( site_is_option_group( $value ) ) {
			foreach ( site_get_changed_keys( $old_value, $value ) as $field_key ) {
				if ( ! site_is_key_ignored( $option, $field_key ) ) {
					$old_value_a = '';
					$new_value_a = '';
					...
					if ( $old_value_a != $new_value_a ) {
						$changed_options[] = array(
							'title'         => site_wpseo_settings_label( $option, $field_key ) ? site_wpseo_settings_label( $option, $field_key ) : '',
			...
			$changed_options[] = array(
				'title'       => site_wpseo_settings_label( $option, '' ) ? site_wpseo_settings_label( $option, '' ) : '',
				'action'      => $option,
				'object_type' => $obj_type,
		...
		foreach ( $changed_options as $properties ) {
			if ( $properties['title'] ) {
				site_get_activity_function( $properties['action'], $properties['object_type'], '', "'" . $properties['title'] . "' setting was updated" );
...
if ( ! function_exists( 'site_wpseo_settings_label' ) ) {
	/**
	 ...
	 */
	function site_wpseo_settings_label( $option, $field_key ) {
...
if ( ! function_exists( 'site_is_option_group' ) ) {
	/**
	 * Fires when WordPress seo settings label.
	 *
	 * @param string $value Value.
	 */
	function site_is_option_group( $value ) {
		if ( ! is_array( $value ) ) {
			return false;
		}
...
if ( ! function_exists( 'site_is_key_ignored' ) ) {
	/**
	 * Fires when WordPress seo settings label.
	 *
	 ...
	 */
	function site_is_key_ignored( $option_name, $key ) {
		$ignored = array(
			'theme_mods' => array(
...
if ( ! function_exists( 'site_get_changed_keys' ) ) {
	/**
	 * Fires when WordPress seo settings label.
	 *
	 * @param string $old_value Old Value.
	 ...
	 */
	function site_get_changed_keys( $old_value, $new_value, $deep = false ) {
		...
		foreach ( $result as $key => $val ) {
			if ( in_array( $key, $unique_keys_old, true ) ) {
				...
				if ( is_array( $old_value[ $key ] ) && is_array( $new_value[ $key ] ) ) {
					...
					$parent = $key;
					$deep--;
					$changed = site_get_changed_keys( $old_value[ $key ], $new_value[ $key ], $deep );
...
if ( ! function_exists( 'site_wpseo_postmeta' ) ) {
	/**
	 ...
	 */
	function site_wpseo_postmeta( $meta_id, $object_id, $meta_key, $meta_value ) {
...
add_action( 'added_post_meta', 'site_wpseo_postmeta', 10, 4 );
add_action( 'updated_post_meta', 'site_wpseo_postmeta', 10, 4 );
add_action( 'deleted_post_meta', 'site_wpseo_postmeta', 10, 4 );
