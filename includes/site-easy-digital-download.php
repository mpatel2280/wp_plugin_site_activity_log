...
if ( ! function_exists( 'site_activate_discount_code' ) ) {
	/**
	 * Get activity for the user - Activate Discount code.
	 *
	 * @param array $data dicount data.
	 */
	function site_activate_discount_code( $data ) {
		if ( $data['discount'] ) {
			$action     = 'Activate';
...
add_action( 'edd_activate_discount', 'site_activate_discount_code' ); // new.

if ( ! function_exists( 'site_deactivate_discount_code' ) ) {
	/**
	 * Get activity for the user - Deactivate Discount code.
	 *
	 * @param array $data dicount data.
	 */
	function site_deactivate_discount_code( $data ) {
...
add_action( 'edd_deactivate_discount', 'site_deactivate_discount_code' ); // new.

if ( ! function_exists( 'site_add_discount_code' ) ) {
	/**
	 * Get activity for the user - Add Discount.
	 *
	 * @param array $data dicount data.
	 */
	function site_add_discount_code( $data ) {
		if ( isset( $data['discount'] ) ) {
			$action     = 'Added';
			$obj_type   = 'edd_discount';
...
add_action( 'edd_add_discount', 'site_add_discount_code' );


if ( ! function_exists( 'site_delete_discount_code' ) ) {
	/**
	 * Get activity for the user - Delete Discount.
	 *
	 * @param array $data dicount data.
	 */
	function site_delete_discount_code( $data ) {
		if ( isset( $data['discount'] ) ) {
			$action     = 'Deleted';
			$obj_type   = 'edd_discount';
			$post_id    = $data['discount'];
...
add_action( 'edd_delete_discount', 'site_delete_discount_code' );

if ( ! function_exists( 'site_edit_discount' ) ) {
	/**
	 * Get activity for the user - Edit Discount.
	 */
	function site_edit_discount() {
		if ( isset( $_POST['edd-discount-nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['edd-discount-nonce'] ) ), 'edd_discount_nonce' ) ) {
			if ( ! empty( $_POST['discount-id'] ) ) {
				...
				$post_meta_data = site_get_post_meta_data( 'discount' );
				$new_post_meta  = '';
